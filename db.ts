...
import { VALID_API_KEYS } from "./index.ts";
...
export function createAlarm(
  ...
  const stmt = db.prepare(
    `INSERT INTO alarms (apiKey, soundId, soundName, alarmTime) VALUES (?, ?, ?, ?)`,
  );
...
export function getAllAlarms(apiKey: string): Alarm[] {
  if (!validateApiKey(apiKey)) throw new Error("Invalid API key");
  const stmt = db.prepare(
    "SELECT * FROM alarms WHERE apiKey = ? ORDER BY alarmTime",
  );
...
export function getActiveAlarmsForTime(apiKey: string, time: string): Alarm[] {
  if (!validateApiKey(apiKey)) throw new Error("Invalid API key");
  const stmt = db.prepare(
    "SELECT * FROM alarms WHERE alarmTime = ? AND isActive = 1 AND apiKey = ?",
  );
